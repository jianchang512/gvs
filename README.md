...
- **🚀 AI 原生处理**：使用智谱 AI (glm-4.6v-flash免费模型) 或 Google Gemini 进行识别（支持自定义Open AI、Gemini和Claude API地址），抗干扰能力强，支持多语言。
- **🆓 免费使用**：
...
6.  **查看结果**：
    ...
    *   点击界面上的“打开输出目录”即可直达。
7. **时间轴校对**：
   生成的字幕可以保证较高准确率，但是时间轴受到识别方式的影响会有一定误差，您可能还需要通过其他软件进行语音对轴。
...
首先通过ffmpeg进行大量截图，然后通过在数组中依次添加文本提示和每张图片的 base64 数据，每个图片对象包含 base64 编码的图像数据和 MIME 类型，这样模型能够按顺序处理所有图像。
