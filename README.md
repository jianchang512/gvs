...
- **🚀 AI 原生处理**：使用智谱 AI (glm-4.6v-flash免费模型) 或 Google Gemini 或自定义 LLM 进行识别（支持自定义Open AI、Gemini和Claude API地址），抗干扰能力强，支持多语言。
- **🆓 免费使用**：
...
### 3. 自定义API
*   **模型**: **需要支持视觉识别的模型**
*   **API地址格式**：直接填写 `https://api.your-proxy.com` 无需`v1`等后缀（请参考中转方提供地址填写）
*   **费用**：由于需要发送大量图片供LLM进行识别，消耗token数可能极高（1分钟视频输入为0.25M左右），且与视频分辨率有关，建议提前压制字幕肉眼识别问题不大的低分辨率视频进行识别或使用非token计数的付费方案（1分钟请求数18次）。
...
3.  **选择区域**：
    *   大多数电影/电视剧字幕在**底部**。
    *   如果是短视频顶部标题，可选**顶部**。
    *   若不确定，可选**全画面**（Token 消耗稍多，但最稳）或**自定义**（拉动**起始滑块**和**结束滑块**调整，仅可调整纵向尺寸部分）。
    *   如果不确定自定义尺寸，可通过网页app：[视频垂直裁剪预览工具](https://cropvideo.t7sint.eu.org/) 预览裁剪尺寸。
4.  **配置服务**：
    *   选择服务商（智谱AI 或 Gemini 或自定义）。
    *   输入对应的 API Key（软件会自动保存，下次无需输入）。
    *   选择模型（推荐使用后缀带 `flash` 的模型，速度快且免费/便宜）。
    *   选择自定义服务商时，请记得填写API地址。
5.  **开始提取**：点击“开始提取”按钮。
6.  **查看结果**：
    *   进度条走完后，软件会自动在当前目录下创建 `output` 文件夹。
    *   SRT 字幕文件将生成在 `output` 目录中。
    *   点击界面上的“打开输出目录”即可直达。
7. **时间轴校对**：生成的字幕可以保证较高准确率，但是时间轴受到识别方式的影响**会有一定误差**，您可能还需要通过其他软件进行语音对轴。
...
*   **AI 逻辑**: 核心逻辑在 `AIClient` 类中。如果想添加新的 LLM 支持，请参照 `_call_zhipu` 方法实现。
